...
// Holds validated bill data while description modal is open
...
function saveBill() {
    ...
    setTimeout(() => document.getElementById('billDescInput').focus(), 400);
}

function doSaveBill() {
    if (!window._pendingBill) return;
    const description = document.getElementById('billDescInput').value.trim();

    // Close modal
    const modalEl = document.getElementById('billDescModal');
    ...
    $.ajax({
        ...
        data: {
            ...
            attenderId:   b.attenderId,
            priceCategory: b.priceCategory,
            isTaxBill:    b.isTaxBill,
            finalDiscount: b.finalDiscount,
