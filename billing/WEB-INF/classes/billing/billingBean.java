...
public String saveBillItems(List<ProductItem> items,
    ...
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, priceCategory, attenderId, isTaxBill, "");
}
...
ps.executeUpdate();
        ...
        // Insert into prod_ledger (is_receipt=1 for billing)
...
public Vector getBillDetailsUsingNo(String bill) throws Exception {
    ...
    try {
        ...
        // Step 1: resolve to a single bill id (guards against duplicate bill_display)
        int billId = 0;
        ps = con.prepareStatement("SELECT id FROM prod_bill WHERE bill_display=? ORDER BY id DESC LIMIT 1");
        ps.setString(1, bill);
        rs = ps.executeQuery();
        if (rs.next()) {
            billId = rs.getInt(1);
        ...
        if (billId == 0) return vec; // bill not found

        // Step 2: fetch bill details using the resolved id
		...
		try
			{
		      ...
		      pt = con.prepareStatement("SELECT extraDisc FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
		      pt.setString(1,bill);
		      rs = pt.executeQuery();
		      if(rs.next())
		      	disc  = rs.getInt(1);
		...
		try
			{
		      ...
		      pt = con.prepareStatement("SELECT paid FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
		      pt.setString(1,bill);
		      rs = pt.executeQuery();
		      if(rs.next())
		      	disc  = rs.getDouble(1);
		...
		try
			{
		      ...
		      pt = con.prepareStatement("SELECT balance FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
		      pt.setString(1,bill);
		      rs = pt.executeQuery();
		      if(rs.next())
		      	disc  = rs.getDouble(1);
...
public String getBillDate(String bill)throws Exception
	...
	con						= util.DBConnectionManager.getConnectionFromPool();
	pt = con.prepareStatement("SELECT CONCAT(DATE_FORMAT(date, '%d-%m-%Y'), ' ', TIME_FORMAT(time, '%H:%i:%s')) AS bill_datetime FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
	pt.setString(1,bill);
	rs=pt.executeQuery();
	if(rs.next())
		{
...
public String getNumPaid(double amount) {
 ...
 public String getCusName(String bill)throws Exception
	...
	con						= util.DBConnectionManager.getConnectionFromPool();
	pt = con.prepareStatement("SELECT cusName FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
	pt.setString(1,bill);
	rs=pt.executeQuery();
	if(rs.next())
		{
...
public Vector getCustomerDetailsByBillNo(String bill) throws Exception {
    ...
    try {
        ...
        pt = con.prepareStatement(
            ...
            "FROM prod_bill b " +
            "LEFT JOIN customers c ON b.customerId = c.id " +
            "WHERE b.id = (SELECT id FROM prod_bill WHERE bill_display = ? ORDER BY id DESC LIMIT 1)"
        );
        pt.setString(1, bill);
        rs = pt.executeQuery();
...
public String getCusNumber(String bill)throws Exception
	...
	con						= util.DBConnectionManager.getConnectionFromPool();
	pt = con.prepareStatement("SELECT cusPhn FROM `prod_bill` WHERE bill_display=? ORDER BY id DESC LIMIT 1;");
	pt.setString(1,bill);
	rs=pt.executeQuery();
	if(rs.next())
		{
...
public Vector getMonthlySalesCharts() throws Exception {
    Connection con = null;
    PreparedStatement pt = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        pt = con.prepareStatement(
            "SELECT DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH), '%b %Y') AS month_label, " +
            "       IFNULL(SUM(pb.payable), 0) AS total_sales " +
            "FROM ( " +
            "  SELECT 11 AS n UNION ALL SELECT 10 UNION ALL SELECT 9 UNION ALL SELECT 8 " +
            ...
            ") AS nums " +
            "LEFT JOIN prod_bill pb " +
            "  ON MONTH(pb.date) = MONTH(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH)) " +
            "  AND YEAR(pb.date) = YEAR(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH)) " +
            "  AND pb.is_cancelled = 0 " +
            "GROUP BY nums.n, DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH), '%b %Y') " +
            "ORDER BY nums.n DESC"
        );
        rs = pt.executeQuery();
        while (rs.next()) {
            Vector row = new Vector();
...
public Vector getYearlySalesData(int year) throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        ps = con.prepareStatement(
            "SELECT nums.m, COALESCE(COUNT(pb.id),0) AS clients, COALESCE(SUM(pb.payable),0) AS total_sales " +
            ...
            "  AND pb.is_new_client=1 AND pb.is_cancelled=0 " +
        ...
        while (rs.next()) {
            ...
            row.addElement(rs.getLong(2));    // client count
            row.addElement(rs.getDouble(3));  // total sales
            vec.addElement(row);
        }
        return vec;
    } finally {
...
public Vector getYearlyExpenseData(int year) throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        ps = con.prepareStatement(
            "SELECT nums.m, COALESCE(SUM(ee.amount),0) AS total_expense " +
            "FROM (SELECT 1 AS m UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 " +
            "      UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 " +
            "      UNION ALL SELECT 9 UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12) AS nums " +
            "LEFT JOIN expense_entry ee ON MONTH(ee.exc_date_time)=nums.m AND YEAR(ee.exc_date_time)=? " +
            "  AND ee.is_active=1 AND ee.exp_type!=2 " +
            "GROUP BY nums.m ORDER BY nums.m"
        );
        ps.setInt(1, year);
        rs = ps.executeQuery();
        while (rs.next()) {
            Vector row = new Vector();
            row.addElement(rs.getInt(1));    // month number 1-12
            row.addElement(rs.getDouble(2)); // total expense
            vec.addElement(row);
        }
        return vec;
    } finally {
...
public Vector getAllYearsSalesData() throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        ps = con.prepareStatement(
            "SELECT YEAR(pb.date) AS yr, COUNT(pb.id) AS clients, COALESCE(SUM(pb.payable),0) AS total_sales " +
            "FROM prod_bill pb WHERE pb.is_new_client=1 AND pb.is_cancelled=0 " +
...
public Vector getAllYearsExpenseData() throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        ps = con.prepareStatement(
            "SELECT YEAR(ee.exc_date_time) AS yr, COALESCE(SUM(ee.amount),0) AS total_expense " +
            "FROM expense_entry ee WHERE ee.is_active=1 AND ee.exp_type!=2 " +
            "GROUP BY yr ORDER BY yr"
        );
        rs = ps.executeQuery();
        while (rs.next()) {
...
public Vector getYearlyServiceData(int year) throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
...
public Vector getAllYearsServiceData() throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
...
public Vector getInitialBillPayment(String billNo) throws Exception {
    ...
    try {
        ...
        String sql = "SELECT DATE_FORMAT(a.date,'%d-%m-%Y'), b.cash, b.bank, "
            ...
            + "a.balance "
            + "FROM prod_bill a JOIN prod_bill_payment b ON b.bill_id=a.id WHERE a.bill_display=? ORDER BY a.id DESC LIMIT 1";
        ps = con.prepareStatement(sql);
        ps.setString(1, billNo);
        rs = ps.executeQuery();
        if (rs.next()) {
