...
public void addExpenseEntry(int expenseType, String content, String description, double amount, String expenseDateTime, int userId) throws Exception {
    ...
    try {
        ...
        // Insert into prod_ledger (is_receipt=0 for expense)
        String sqlLedger = "INSERT INTO prod_ledger (entry_date_time, content, amount, is_receipt, ref_id, uid) VALUES (?, ?, ?, 0, ?, ?)";
        pt = con.prepareStatement(sqlLedger);
        pt.setString(1, expenseDateTime);
        pt.setString(2, content);
        pt.setDouble(3, amount);
        pt.setInt(4, expenseEntryId);
        pt.setInt(5, userId);
        pt.executeUpdate();
...
//////////////////////////----------------------------
// Returns opening balance (before fromDate) as first element, then each ledger row:
// [entry_date_time, content, amount, is_receipt]
public Vector getLedgerReport(String fromDate, String toDate) throws Exception {
    Connection con = null;
    PreparedStatement pt = null;
    ResultSet rs = null;
    ...
    try {
        ...
        // Opening balance = sum of receipts - sum of expenses before fromDate
        String sqlOpening = "SELECT " +
            "COALESCE(SUM(CASE WHEN is_receipt=1 THEN amount ELSE 0 END), 0) - " +
            "COALESCE(SUM(CASE WHEN is_receipt=0 THEN amount ELSE 0 END), 0) " +
            "FROM prod_ledger WHERE DATE(entry_date_time) < ?";
        pt = con.prepareStatement(sqlOpening);
        pt.setString(1, fromDate);
        rs = pt.executeQuery();
        ...
        Vector openingRow = new Vector();
        openingRow.add(openingBalance);
        result.add(openingRow);

        // Ledger rows within date range
        String sqlRows = "SELECT entry_date_time, content, amount, is_receipt " +
            "FROM prod_ledger " +
            "WHERE DATE(entry_date_time) BETWEEN ? AND ? " +
            "ORDER BY entry_date_time ASC, id ASC";
        pt = con.prepareStatement(sqlRows);
        pt.setString(1, fromDate);
        pt.setString(2, toDate);
        rs = pt.executeQuery();
        while (rs.next()) {
            Vector row = new Vector();
            row.add(rs.getString("entry_date_time"));
            row.add(rs.getString("content"));
            row.add(rs.getDouble("amount"));
            row.add(rs.getInt("is_receipt"));
            result.add(row);
        }
    } catch (Exception e) {
        System.err.println("Error in getLedgerReport: " + e.getMessage());
        e.printStackTrace();
        throw e;
    } finally {
...
public Vector getExpenseReportCharts() throws Exception {
    Connection con = null;
    PreparedStatement pt = null;
    ResultSet rs = null;
    try {
        ...
        Vector vec = new Vector();
        pt = con.prepareStatement(
            "SELECT DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL nums.n DAY), '%d-%m-%Y') AS expense_date, " +
            "       IFNULL(SUM(ee.amount), 0) AS total_expense " +
            "FROM ( " +
            ...
            ") AS nums " +
            "LEFT JOIN expense_entry ee " +
            "  ON DATE(ee.exc_date_time) = DATE_SUB(CURDATE(), INTERVAL nums.n DAY) " +
            "  AND ee.is_active = 1 " +
            "GROUP BY nums.n " +
            "ORDER BY nums.n ASC"
        );
...
public Vector getMonthlyExpenseCharts() throws Exception {
    Connection con = null;
    PreparedStatement pt = null;
    ResultSet rs = null;
    try {
        ...
        pt = con.prepareStatement(
            ...
            ") AS nums " +
            "LEFT JOIN expense_entry ee " +
            "  ON MONTH(ee.exc_date_time) = MONTH(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH)) " +
            "  AND YEAR(ee.exc_date_time) = YEAR(DATE_SUB(CURDATE(), INTERVAL nums.n MONTH)) " +
            "  AND ee.is_active = 1 " +
